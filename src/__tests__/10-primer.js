...
import { findAllInRenderedTree, isCompositeComponentWithType } from 'react-dom/test-utils';
...
const findToggleInstances = rootInstance =>
  findAllInRenderedTree(rootInstance, c => isCompositeComponentWithType(c, Toggle));
...
test('toggling either toggle toggles both', () => {
  ...
  const { container, rootInstance } = renderToggle(<Usage onToggle={handleToggle} />);
...
test('toggle can still be uncontrolled', () => {
  const handleToggle = jest.fn();
  const { toggleButton, toggle } = renderToggle(<Toggle onToggle={handleToggle} />);
...
/*
http://ws.kcd.im/?ws=react%20patterns&e=10-primer&em=johan.c.quiroga@gmail.com
*/
