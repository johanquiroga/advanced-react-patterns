  ...
  getState = (state = this.state) => {
    return Object.keys(state).reduce((combinedState, key) => {
      if (this.isControlled(key)) {
        ...
        combinedState[key] = this.state[key];
      }
  ...
  internalSetState(changes, callback = () => {}) {
    ...
    this.setState(
      state => {
        ...
        const nonControlledChanges = Object.keys(combinedState).reduce((stateChanges, key) => {
          if (!this.isControlled(key)) {
            stateChanges[key] = allChanges[key] || combinedState[key];
          }
          return stateChanges;
        }, {});
...
class Usage extends React.Component {
  ...
  handleStateChange = changes => {
    if (changes.type === 'forced') {
      this.setState({ toggleOn: changes.on }, () => this.props.onToggle(this.state.toggleOn));
  ...
  render() {
    ...
    return (
      <Toggle on={toggleOn} onStateChange={this.handleStateChange} ref={this.props.toggleRef}>
